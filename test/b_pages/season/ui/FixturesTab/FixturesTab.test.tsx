/**
 * FixturesTab 통합 테스트 — QA 시나리오(플랜 "SeasonTabs 탭 전환·필터 적용 시
 * 행 수 변화·대회필터로 빈상태 도달").
 *
 * 검증 목적:
 * - 초기 렌더 = 전체 13경기
...
import { FixturesTab } from '@pages/season/ui/FixturesTab';
...
describe('FixturesTab', () => {
  it('초기 렌더 시 전체 경기를 모두 렌더한다(필터=전체)', () => {
    render(<FixturesTab />);
    expect(rowCount()).toBe(fixtures.length);
  });
  ...
  it('홈 필터를 선택하면 홈경기 수만큼만 렌더된다', async () => {
    ...
    const homeCount = fixtures.filter((f) => f.ha === 'home').length;
  ...
  it('대회 필터(FA컵)를 선택하면 FA컵 경기 수만큼만 렌더된다', async () => {
    ...
    const faCupCount = fixtures.filter((f) => f.comp === 'FA컵').length;
  ...
  it('홈 + FA컵처럼 결과가 없는 필터 조합이면 빈 상태(StateBox)가 렌더된다', async () => {
    ...
    expect(screen.getByRole('heading', { name: '조건에 맞는 경기가 없어요' })).toBeInTheDocument();
