 ...
 *
 * 검증 목적:
 * - FLAG_GRADIENT에 매핑된 code(eng — 감독 국적)는 국기 그라디언트 배경으로 렌더된다
 * - 매핑에 없는 code를 주입했을 때 muted 폴백 배경으로 대체되고 크래시 없다
 *   (FlagSwatch.tsx 미도달 분기)
 */
...
describe('FlagSwatch', () => {
  it('매핑된 code(eng)는 국기 그라디언트 배경으로 렌더된다', () => {
    const { container } = render(<FlagSwatch code="eng" />);
    const swatch = container.querySelector('span[aria-hidden="true"]');
    expect(swatch).not.toBeNull();
    expect(swatch).toHaveStyle({
      background: 'linear-gradient(90deg,#fff 0 38%,#ce1126 38% 62%,#fff 62% 100%)',
    });
  });
