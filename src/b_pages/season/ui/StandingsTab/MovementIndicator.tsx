...
import type { StandingMovement } from '../../model';
...
const MOVEMENT_CONFIG: Record<StandingMovement, MovementConfig> = {
  up: { icon: <ArrowUp size={14} aria-hidden="true" />, colorClassName: 'text-win', label: '순위 상승' },
  down: { icon: <ArrowDown size={14} aria-hidden="true" />, colorClassName: 'text-united-red', label: '순위 하락' },
  same: { icon: <Minus size={14} aria-hidden="true" />, colorClassName: 'text-muted-foreground', label: '순위 변동 없음' },
};
...
export function MovementIndicator({ movement, className }: MovementIndicatorProps) {
  ...
  return (
    <span
      ...
      className={cn('grid h-[30px] w-[30px] place-items-center', config.colorClassName, className)}
    >
      {config.icon}
    </span>
  );
