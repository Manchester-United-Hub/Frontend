...
import { subTabs } from '@pages/season/model';

afterEach(cleanup);

describe('SubTabNav', () => {
  it('role=tablist와 2개의 role=tab을 렌더한다', () => {
    render(<SubTabNav tabs={subTabs} activeId="fixtures" onChange={() => {}} />);
    expect(screen.getByRole('tablist', { name: '시즌 하위 탭' })).toBeInTheDocument();
  ...
  it('탭 클릭 시 onChange가 해당 탭 id로 호출된다', async () => {
    ...
    render(<SubTabNav tabs={subTabs} activeId="fixtures" onChange={onChange} />);
