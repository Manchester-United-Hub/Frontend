 ...
 *
 * 검증 목적: utd 행 강조(배경·팀명 굵게), 득실차 색 분기(양수=win·음수=
 * united-red — 득실차 텍스트는 diff 필드 값 그대로 렌더되고 별도의 "+" 접두사는
 * 붙지 않는다, 음수는 숫자 자체의 "-"로만 표시됨을 실제 렌더로 확인), 승점 렌더,
 * 최근 5경기 폼 배지 5개, zone별 posbar 색
...
import { StandingsRow } from '@entities/rank/ui/StandingsRow';
...
describe('StandingsRow', () => {
  ...
  it('득실차가 양수면 win 색으로 렌더된다(diff 값은 부호 접두사 없이 그대로 표시)', () => {
    const positive = standings.find((s) => s.gf - s.ga > 0)!;
    renderRow(positive);
    expect(screen.getByText(String(positive.diff))).toHaveClass('text-win');
  });

  it('득실차가 음수면 united-red 색으로 렌더된다(부호는 숫자 자체의 "-")', () => {
    const negative = standings.find((s) => s.gf - s.ga < 0)!;
    renderRow(negative);
    expect(screen.getByText(String(negative.diff))).toHaveClass(
      'text-united-red'
    );
  });

  it('득실차가 0이면 색상 강조 클래스 없이 렌더된다(gf===ga, mock 데이터에는 없는 방어 케이스)', () => {
    const evenGoalDiff: Standing = { ...standings[0], gf: 30, ga: 30, diff: 0 };
  ...
  it('zone=uel/conf는 raw hex 인라인 style로 렌더된다(대응 토큰 부재)', () => {
    ...
    const { container: uelContainer } = renderRow(uelStanding);
    expect(uelContainer.querySelector('span.absolute')).toHaveStyle({
      backgroundColor: '#2a6fdb',
    });

    const confStanding = standings.find((s) => s.zone === 'conf')!;
    const { container: confContainer } = renderRow(confStanding);
    expect(confContainer.querySelector('span.absolute')).toHaveStyle({
