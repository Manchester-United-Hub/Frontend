...
import { fixtures, useFixtureFilters, type CompFilter, type HaFilter } from '../../model';
...
const PANEL_DESCRIPTION = '2025/26 시즌 프리미어리그·FA컵·챔피언스리그 일정과 결과를 확인하세요.';
const EMPTY_ICON = <CalendarX size={22} aria-hidden="true" />;

/**
 * FixturesTab — 일정 & 결과 탭. `useFixtureFilters`(ST-01)로 홈/원정·대회 필터
...
export function FixturesTab() {
  const { ha, comp, setHa, setComp, groups, isEmpty } = useFixtureFilters(fixtures);

  return (
    <Shell className="pb-16 pt-10">
      <PanelHead eyebrow="Fixtures & Results" title="일정 & 결과" description={PANEL_DESCRIPTION} />

      <div className="mb-6 flex flex-wrap gap-6">
        <FilterPills label="홈/원정" options={HA_OPTIONS} value={ha} onChange={setHa} />
        <FilterPills label="대회" options={COMP_OPTIONS} value={comp} onChange={setComp} />
      </div>
