...
import { getNewsPage } from '@pages/newsArticle/model';
import type { ArticleItem, NewsQuery } from '@pages/newsArticle/model';

afterEach(cleanup);

const makeArticles = (count: number): ArticleItem[] =>
...
describe('NewsArticlePage', () => {
  it('최초 로딩 후 첫 페이지 기사와 제목을 렌더한다', async () => {
    render(<NewsArticlePage source={sourceFrom(makeArticles(4))} pageSize={2} fetchDelayMs={0} />);
  ...
  it('기사가 없으면 빈 상태를 렌더한다', async () => {
    render(<NewsArticlePage source={sourceFrom([])} pageSize={2} fetchDelayMs={0} />);
