...
import { getCurrentSeason } from '@entities/season/api/server/fetchCurrentSeason';
...
describe('getCurrentSeason', () => {
  ...
  it('응답을 ServerApiResult 형태로 반환한다', async () => {
    const data = { season: 2026, started: true };
    const mockRes = {
      ok: true,
      status: 200,
    ...
    expect(serverFetcher.get).toHaveBeenCalledWith('/api/seasons/current');
