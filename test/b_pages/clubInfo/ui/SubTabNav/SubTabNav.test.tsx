 ...
 * 컴포넌트 1:테스트 1 미러링 완성.
 *
 * 검증 목적: role=tablist/tab 렌더, aria-selected, "준비 중" 뱃지 1개(soon, 팀통계),
...
describe('SubTabNav', () => {
  it('role=tablist와 4개의 role=tab을 렌더한다', () => {
  ...
  it('soon 탭(팀통계)에만 "준비 중" 뱃지가 렌더된다(1개)', () => {
    render(<SubTabNav tabs={subTabs} activeId="history" onChange={() => {}} />);
  ...
  it('각 탭은 id="tab-{id}"·aria-controls="panel-{id}" 접근성 계약을 만족한다', () => {
    render(<SubTabNav tabs={subTabs} activeId="history" onChange={() => {}} />);
    const managerTab = screen.getByRole('tab', { name: /감독/ });
    expect(managerTab).toHaveAttribute('id', 'tab-manager');
    expect(managerTab).toHaveAttribute('aria-controls', 'panel-manager');
  });
});
