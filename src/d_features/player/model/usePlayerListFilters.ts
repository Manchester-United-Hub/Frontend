 ...
 * pageResults는 results를 pageSize 인자로 자른 현재 페이지 조각이다 — 페이지네이션은
 * 서버가 아니라 여기서 한다(필터·검색이 클라이언트에서 전체 스쿼드를 대상으로 돌기 때문에,
 * 서버 페이징과 섞으면 페이지 밖 선수가 검색·필터에서 누락된다). pageSize는 useRosterPageSize가
 * 뷰포트에 따라 계산해 호출부가 주입한다(A-8/S-13) — 이 훅 내부에서는 useRosterPageSize를
 * 직접 호출하지 않는다. 시그니처에 드러나지 않는 브라우저 의존성이 생기면 matchMedia 스텁 없이
 * 이 훅을 단위 테스트할 수 없기 때문이다.
 * refresh는 700ms 로딩 시뮬레이션(REFRESH_DELAY_MS) — 언마운트 시 clearTimeout 필수(ADR-8).
 */

import { useCallback, useEffect, useMemo, useRef, useState } from 'react';

import { ALL_FILTER_KEY, filterPlayers } from '@entities/player/model';
...
const usePlayerListFilters = (
  players: PlayerListItem[],
  pageSize: number,
): UsePlayerListFiltersResult => {
  ...
  const totalPages = Math.max(FIRST_PAGE, Math.ceil(results.length / pageSize));
  // 재조회로 목록이 줄어 현재 페이지가 사라지면 마지막 페이지로 당긴다 — effect 없이 렌더 중 보정.
  const page = Math.min(requestedPage, totalPages);

  const pageResults = useMemo(
    () => results.slice((page - FIRST_PAGE) * pageSize, page * pageSize),
