...
/** 필터 pill 버튼 1개. 모듈 스코프 정의(FilterPills 내부 정의 금지 — code-conventions §1). */
function FilterPillButton<T extends string>({ option, isActive, onChange }: FilterPillButtonProps<T>) {
  ...
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      <div role="group" aria-label={label} className="flex flex-wrap gap-1.5">
        {options.map((option) => (
          <FilterPillButton key={option.value} option={option} isActive={option.value === value} onChange={onChange} />
        ))}
      </div>
    </div>
  );
