...
import { PanelHead } from '@shared/ui/Panel';

afterEach(cleanup);

describe('PanelHead', () => {
  it('eyebrow와 title(h2)을 렌더한다', () => {
    render(<PanelHead eyebrow="Matches & Results" title="일정 & 결과" />);
    expect(screen.getByText('Matches & Results')).toBeInTheDocument();
    expect(
  ...
  it('description이 없으면 문단이 렌더되지 않는다', () => {
    const { container } = render(
      <PanelHead eyebrow="League Table" title="순위표" />
    );
  ...
  it('headingId를 전달하면 h2의 id로 그대로 반영된다', () => {
    ...
    ).toHaveAttribute('id', 'table-heading');
  });
});
