...
import { lineupSchema } from '@entities/matches/types';

const {
  CoachDTOSchema,
  EventDTOSchema,
  LineupDTOSchema,
  LiveMatchLineupDTOSchema,
  PastMatchDetailDTOSchema,
  PlayerDTOSchema,
...
describe('PastMatchDetailDTOSchema', () => {
  it('lineups와 events가 있으면 통과한다', () => {
    const data = { lineups: [validLineup], events: [validEvent] };
    expect(PastMatchDetailDTOSchema.safeParse(data).success).toBe(true);
  });
  it('빈 배열이면 통과한다', () => {
    expect(
      PastMatchDetailDTOSchema.safeParse({ lineups: [], events: [] }).success
    ).toBe(true);
  });
});

describe('LiveMatchLineupDTOSchema', () => {
  it('matchId와 lineups가 있으면 통과한다', () => {
    const data = { matchId: 100, lineups: [validLineup] };
    expect(LiveMatchLineupDTOSchema.safeParse(data).success).toBe(true);
  });
  it('matchId가 없으면 실패한다', () => {
    expect(LiveMatchLineupDTOSchema.safeParse({ lineups: [] }).success).toBe(
      false
    );
  });
});
